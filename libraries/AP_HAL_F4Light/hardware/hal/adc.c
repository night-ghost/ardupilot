...
#define CR_CLEAR_MASK             ((uint32_t)0xFFFC30E0) // Clear MULTI, DELAY, DMA and ADCPRE bits
#define CR1_CLEAR_MASK            ((uint32_t)0xFCFFFEFF) // Clear RES and SCAN bits 
#define CR2_CLEAR_MASK            ((uint32_t)0xC0FFF7FD) // Clear CONT, ALIGN, EXTEN and EXTSEL bits 

void adc_init(const adc_dev *dev) {
 
    RCC_doAPB2_reset(RCC_APB2_bit_ADC); // turn on clock and do reset to all ADCs
      
    uint32_t tmp = ADC->CCR  & CR_CLEAR_MASK; 
