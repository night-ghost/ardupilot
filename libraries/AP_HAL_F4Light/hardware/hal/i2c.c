...
void i2c_init(const i2c_dev *dev, uint16_t address, uint32_t speed)
{
    ...
    if (speed <= 100000) {    // Standard mode speed calculate
        ...
        dev->regs->CCR = (uint16_t)(ccr | I2C_CCR_FS); // Set speed value and F/S bit for fast mode 
        dev->regs->TRISE = (uint16_t)((freq * 300 / 1000) + 1); // Set Maximum Rise Time for fast mode 
    }
    i2c_peripheral_enable(dev);

    reg = dev->regs->CR1 & I2C_CR1_CLEAR_MASK;   // Clear ACK, SMBTYPE and  SMBUS bits 
    
    dev->regs->CR1 = reg | (uint16_t)(I2C_Mode_I2C | I2C_Ack_Enable); // mode and acknowledgement 
