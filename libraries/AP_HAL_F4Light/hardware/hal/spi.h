...
typedef enum spi_mode {
    SPI_MODE_0=0,  // SCK line idle low  (0), data captured on first  clock transition
    SPI_MODE_1,    // SCK line idle low  (0), data captured on second clock transition
    SPI_MODE_2,    // SCK line idle high (1), data captured on first clock transition
    SPI_MODE_3     // SCK line idle high (1), data captured on second clock transition
} spi_mode;
...
typedef enum spi_baud_rate {
    SPI_BAUD_PCLK_DIV_2   = ((uint16_t)0),    // f_PCLK/2
    SPI_BAUD_PCLK_DIV_4   = ((uint16_t)1<<3), // f_PCLK/4
    SPI_BAUD_PCLK_DIV_8   = ((uint16_t)2<<3), // f_PCLK/8
    SPI_BAUD_PCLK_DIV_16  = ((uint16_t)3<<3), // f_PCLK/16
    SPI_BAUD_PCLK_DIV_32  = ((uint16_t)4<<3), // f_PCLK/32
    SPI_BAUD_PCLK_DIV_64  = ((uint16_t)5<<3), // f_PCLK/64
    SPI_BAUD_PCLK_DIV_128 = ((uint16_t)6<<3), // f_PCLK/128
    SPI_BAUD_PCLK_DIV_256 = ((uint16_t)7<<3), // f_PCLK/256
} spi_baud_rate;



// SR1 bits
#define SPI_BIT_RXNE               ((uint16_t)1<<0)
#define SPI_BIT_TXE                ((uint16_t)1<<1)
#define SPI_BIT_CHSIDE             ((uint16_t)1<<2)
#define SPI_BIT_UDR                ((uint16_t)1<<3)
#define SPI_BIT_CRCERR             ((uint16_t)1<<4)
#define SPI_BIT_MODF               ((uint16_t)1<<5)
#define SPI_BIT_OVR                ((uint16_t)1<<6)
#define SPI_BIT_BSY                ((uint16_t)1<<7)
#define SPI_BIT_TIFRFE             ((uint16_t)1<<8)


// CR2 bits
#define SPI_DMAreq_Tx              ((uint16_t)1<<1)
#define SPI_DMAreq_Rx              ((uint16_t)1<<0)
...
// CR1 bits
#define SPI_BIT_CPHA                  ((uint16_t)1<<0)
#define SPI_BIT_CPOL                  ((uint16_t)1<<1)
#define SPI_BIT_MSTR                  ((uint16_t)1<<2)
// 3..5 - BR
#define SPI_BIT_SPE                   ((uint16_t)1<<6)
#define SPI_BIT_LSBFIRST              ((uint16_t)1<<7)
#define SPI_BIT_SSI                   ((uint16_t)1<<8)
#define SPI_BIT_SSM                   ((uint16_t)1<<9)
#define SPI_BIT_RXONLY                ((uint16_t)1<<10)
#define SPI_BIT_DFF                   ((uint16_t)1<<11)
#define SPI_BIT_CRCNEXT               ((uint16_t)1<<12)
#define SPI_BIT_CRCEN                 ((uint16_t)1<<13)
#define SPI_BIT_BIDIOE                ((uint16_t)1<<14)
#define SPI_BIT_BIDIMODE              ((uint16_t)1<<15)
...
#define I2SCFGR_CLEAR_MASK            ((uint16_t)0xF040) // don't touch reserved bits
